...
void *cam_capt(void *arg)
{
    	...
    	if(isReady)
   	{
		...
		TCP_Socket cam_socket; 
		cam_socket.TCPcreateClient("192.168.1.14",2018); //地面站ip
		...
		char buff0[1024*1024];
		//Camera_Data *cam_data;
		struct timeval cam_capt_time;
		printf("222 \n");
		while(1)
		{
       			//void *buff0 = malloc(1024 * 1024);  
       			//sprintf(pic_str,"./pic%d.jpg",cam_count);

       			//gettimeofday(&cam_capt_time,NULL);
       			int size0 = read(fd0, buff0, 1024 * 1024);
			printf("size:%d \n",size0);
			//cam_data->size = size0;
			//cam_data->id_num = cam_count;

			//memcpy(cam_data->buff,buff0,size0);

			//pthread_mutex_lock(&time_mutex);
			//cam_data->timestamp = (cam_capt_time.tv_sec-first_time.tv_sec)*1000000+cam_capt_time.tv_usec-first_time.tv_usec;
			//pthread_mutex_unlock(&time_mutex);

			int send_len = cam_socket.TCPSendPackets(buff0, size0);
			printf("send_len%d \n",send_len);
/*
 ...
 */
		...
		close(cam_socket.Get_Client_SocketFD());
   	}
...
int main()
{
	...
	Socket data_socket; 
	data_socket.UDPCreateClient("IPV4","UDP","192.168.1.14",2006); //地面站ip
...
*/
	...
	{	
		...
		if(count_num>10)
		{
			...
			{
				//fwrite(&pkt_data,sizeof(Packet),1,binaryfileID);
				data_socket.UDPSendPackets(&pkt_data, sizeof(Packet));
			}
		}
